admin-quick-info-loading = Загрузка информации об сущности {ESCAPE($entity)}...
...
admin-quick-info-character-identity = Персонаж: [color=white]{ESCAPE($character)}[/color] (Идентичность: [color=white]{ESCAPE($identity)}[/color])
admin-quick-info-job = Начальная должность: [color=white]{ESCAPE($job)}[/color]
admin-quick-info-role-type-subtype = Роль-подтип: [color={ESCAPE-PARAM($color)}]{ESCAPE($symbol)} {ESCAPE($type)} - {ESCAPE($subtype)}[/color]
admin-quick-info-role-type = Роль: [color={$color}]{ESCAPE($symbol)} {ESCAPE($type)}[/color]
admin-quick-info-no-role = Роль отсутствует
